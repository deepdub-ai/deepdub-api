...
Visit your [Deepdub dashboard](https://app.deepdub.ai) to generate an API key.
...
- Store API keys in a secrets manager (e.g., HashiCorp Vault, or your cloud provider's secrets manager)
...
These limits apply to the public queue. If you have a dedicated model, your concurrency is reserved separately and is not subject to these limits. See [Concurrency & Capacity](/concurrency).
...
<Warning>
  The EU region is **not available** with the free trial API key. To access the EU region, contact your Deepdub representative for a production API key.
</Warning>
