...
## `end-stream`, `cancel`, and `ping`

<ParamField body="end-stream" type="action">
  Flushes any remaining buffered text and marks the current turn as finished. The last audio chunk of the turn is tagged with `"isFinal": true`.
</ParamField>

<ParamField body="cancel" type="action">
  Aborts in-flight generation and clears all buffers. The server emits a synthetic finish message with `"isCancelled": true`.
</ParamField>

<ParamField body="ping" type="action">
  Keepalive. The server replies with `{"action": "pong"}`.
</ParamField>
...
<ResponseField name="isFinal" type="boolean">
  `true` on the final chunk of the whole turn (after `end-stream` or the final flush).
</ResponseField>
