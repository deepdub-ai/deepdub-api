...
1. **Flushes** any text still held in the buffer, so the tail of your turn is synthesized even if it was shorter than a normal batch.
...
<Info>
  You must send `end-stream` to reliably receive the tail of your text. If you stop sending `stream-text` messages without an `end-stream`, buffered text may never be flushed and you will not receive an `isFinal` chunk.
</Info>
