...
<ParamField body="format" type="string" default="wav">
  Output audio format: `wav` (default), `mp3`, `opus`, `mulaw`, or `s16le`. Unlike the [single-request endpoint](/api-reference/websocket/overview), this one does not reject unknown values — an unrecognized format silently produces `wav`, so check your spelling.
