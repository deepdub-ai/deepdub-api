...
<ParamField body="base_websocket_streaming_url" type="string" default="wss://wss.deepdub.ai/ws">
  Base URL for the **streaming text input** WebSocket endpoint — used by `async_stream_connect()` when forwarding LLM tokens incrementally. This is a separate endpoint from `base_websocket_url`. Falls back to `DEEPDUB_BASE_WEBSOCKET_STREAMING_URL` environment variable. See [Streaming Text Input](/streaming-text-input).
</ParamField>

<ParamField body="eu" type="boolean" default="false">
  Use EU region endpoints (`eu-restapi.deepdub.ai`, `wsapi.eu.deepdub.ai`). Falls back to `DD_EU` environment variable (`"1"` to enable).
</ParamField>
...
| **EU** | `https://eu-restapi.deepdub.ai/api/v1` | `wss://wsapi.eu.deepdub.ai/open` |
...
<ParamField body="model" type="string" default="dd-etts-3.3">
  Model ID. Available models: `dd-etts-3.3` (current), `dd-etts-3.0`, `dd-etts-2.5` (legacy).
</ParamField>
...
<ParamField body="duration" type="float">
  ...
  <Warning>
    `duration` / `targetDuration` is no longer actively maintained. Use a slow or fast variant `voicePromptId` for the relevant speaker instead. Existing integrations will continue to work.
  </Warning>
</ParamField>
...
<ParamField body="tempo" type="float">
  ...
  <Warning>
    `tempo` is no longer actively maintained. Use a slow or fast variant `voicePromptId` for the relevant speaker instead. Existing integrations will continue to work.
  </Warning>
</ParamField>
...
For account-level concurrency planning (the reserved capacity on a dedicated model, as opposed to multiplexed requests on a single connection), see [Concurrency & Capacity](/concurrency).
...
<Note>
  The SDK accepts `name` (string) and lowercase `gender` (`'male'`/`'female'`). The underlying REST API uses `title` and uppercase enum (`MALE`/`FEMALE`/`NON_BINARY`). The SDK does not currently expose `NON_BINARY` — use the REST API directly if you need it.
</Note>
...
|----------|-------------|
| `dd-etts-3.3` | Current default — best quality, recommended for all new integrations |
| `dd-etts-3.0` | Stable |
