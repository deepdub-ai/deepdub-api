...
<Note>
  This endpoint synthesizes one complete text per request. To feed text in incrementally as it is produced — for example from an LLM — use the [Live Streaming API](/api-reference/websocket/streaming) instead.
</Note>
...
| **Incremental text input** | Not supported | Not supported here — use the [Live Streaming API](/api-reference/websocket/streaming) |
