...
<ParamField body="model" type="string" required>
  Model ID to use for generation (e.g., `dd-etts-3.3`).
</ParamField>
...
async function streamingTts() {
  ...
  const chunks = [];
  const buffer = await deepdub.generateToBuffer("Hello, this is streamed text input.", {
    locale: "en-US",
    voicePromptId: "bd1b00bb-be1c-4679-8eaa-0fcbfd4ff773",
    model: "dd-etts-3.3",
    format: "wav",
  ...
  console.log(`Total audio: ${buffer.length} bytes`);
}
