...
<Warning>
  These parameters are for experimentation only. Do not set `tempo`, `variance`, `temperature`, `seed`, or `promptBoost` in production voice-agent integrations — they cause the model to produce inconsistent or hallucinated output. See [Production Best Practices](/production-best-practices) for details.
</Warning>
...
audio = client.tts(
    ...
    model="dd-etts-3.2",
    locale="en-US",
    # Experimental parameters — do not use in production:
    temperature=0.7,
    tempo=1.1,
    variance=0.6,
    sample_rate=44100,
    format="mp3",
    prompt_boost=True,
    # Accent blending (safe for production):
